...
Trigger.dev v4 peut être déployé de façon **self-hostable** (limitations documentées vs cloud managé). Le monorepo Video-AI mélangeait auparavant **API HTTP** et **code des tasks** sous `apps/api`, ce qui prêtait à confusion avec la **plateforme** Trigger (webapp, workers, registry, Compose upstream).

## Decision

1. **`apps/trigger`** — **workspace / package tasks** uniquement : `trigger.config.ts`, `src/trigger/*`, scripts `trigger:dev` / `trigger:deploy`, deps SDK (+ `@trigger.dev/build` si requis par le flux de build/deploy validé).
...
3. **`apps/api`** — client HTTP : `tasks.trigger()` avec **import de types uniquement** depuis le package `trigger` ; **bundle runtime sans code des tasks** ; `TRIGGER_API_URL` + `TRIGGER_SECRET_KEY` pour pointer vers l’instance **self-hostable** (ou cloud le temps d’un spike).
