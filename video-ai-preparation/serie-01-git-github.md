...
**Chrome partagé (pilots 01 & 02)** : matrice texte canonique `packages/skills/thp-video-generation/references/library-matrix.md`, `solarTheme`, `SceneHeader` + `ProgressBar`, entrées **`FadeSlide`** (catalogue), `ParticleField` à densité harmonisée ; shell **`Serie01SceneShell`** pour factoriser header + slide (voir `apps/remotion/.../serie-01/Serie01SceneShell.tsx`).
...
**Vidéo suivante dans l’ordre série** : **Merge** (vidéo 5) — créer `pilot-05-merge-outline.md` quand le script est prêt.
